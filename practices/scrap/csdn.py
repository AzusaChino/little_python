...
def init_parser():
    ...
    parser.add_argument('-m', '--method', type=str, dest='method', help='使用方式: download下载 upload上传到草稿箱',
...
def download():
    ...
    if article_url != '-1':
        ...
        try:
            ...
            logging.error('exception raised, fail to get blog \'%s\', exception=%s.', article_url, e)
        finally:
            return
...
def upload_request(headers, content, filename):
    ...
    try:
        ...
        logging.info('upload success, filename=%s, jj_draft_id=%s, jj_draft_url=%s', filename, draft_id,
                     jj_draft_url_tpl % draft_id)
    except Exception as e:
